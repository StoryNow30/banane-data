...
function chunkMap(raws){
  const m=new Map();
  for(const [source,raw] of raws){
    for(const c of raw.clouds||[]){
      const sig=sha256(Buffer.from(JSON.stringify({pointsSceneRelative:c.pointsSceneRelative,visibleByClipBoxes:c.visibleByClipBoxes??null}),'utf8'));
      const prev=m.get(c.chunkId);
      if(prev&&prev.sig!==sig)throw Error(`non-identical duplicate chunk ${c.chunkId}`);
      if(!prev)m.set(c.chunkId,{chunk:c,sig,sources:[source]}); else prev.sources.push(source);
    }
  }
  return m;
}
function captureFor(cap,raw,chunks){
  ...
  const pts=[],vis=[],chunkSources=[];
  for(const id of ge.chunkIds||[]){const hit=chunks.get(id);if(!hit)throw Error(`chunk ${id}`);const c=hit.chunk;chunkSources.push({chunkId:id,sources:[...hit.sources]});for(let i=0;i<c.pointsSceneRelative.length;i++){pts.push(c.pointsSceneRelative[i]);vis.push(c.visibleByClipBoxes?c.visibleByClipBoxes[i]:true);}}
  return{capture:{rails:{[cap.key.side]:snap.rail},pointsSceneRelative:pts,visibleByClipBoxes:vis},chunkSources};
}
const regs=registry(),raws=rawMap(),chunks=chunkMap(raws),rows=[];
...
const body={format:'banane-rsf-failure-families-materialized-v1',methodCommit:'4005aa4569bd597d3b15be6bc35482a187f547a4',dataCommit:'d541686d3a98569125cdbdb261ef121c9f533d6a',role:'frozen RSF V1 descriptive taxonomy applied to the 63 failure payloads re-read from materialized source data; chunks are resolved globally across the nine consulted exports; no human fields',counts,rows};
