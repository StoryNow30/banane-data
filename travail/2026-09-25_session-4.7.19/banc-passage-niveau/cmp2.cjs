...
const P=(process.argv[3]||'abcd').split('');const off=load('off').filter(r=>P.includes(r.set)),on=load(process.argv[4]||'on').filter(r=>P.includes(r.set)),key=r=>r.source+':'+r.cut;
