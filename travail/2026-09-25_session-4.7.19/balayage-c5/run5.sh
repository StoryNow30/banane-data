#!/bin/bash
# Variante « cible » (mesure seulement) : la paire la plus proche de l écartement des voisins, sans garde.
...
for cfg in "cible:--option gaugeGuardMm=1000 --option gaugeTargetStudy=true"; do
  name=${cfg%%:*}; O=${cfg#*:}
  [ -f $S/sweep/$name-done ] && continue
  echo "$(date +%T) $name début" >> $S/sweep/progress5.log
  ...
  touch $S/sweep/$name-done; echo "$(date +%T) $name fin" >> $S/sweep/progress5.log
done
echo FIN >> $S/sweep/progress5.log
