#!/bin/bash
# Filtres du choix, complément : minTop 5, et minTop 10 avec minFace 2 (règles 4.7.15).
...
for cfg in "c15-minTop5:--option chainMm=15 --option minTop=5" "c15-minTop10-minFace2:--option chainMm=15 --option minTop=10 --option minFace=2"; do
  name=${cfg%%:*}; O=${cfg#*:}
  [ -f $S/sweep/$name-done ] && continue
  echo "$(date +%T) $name début" >> $S/sweep/progress6.log
  ...
  touch $S/sweep/$name-done; echo "$(date +%T) $name fin" >> $S/sweep/progress6.log
done
echo FIN >> $S/sweep/progress6.log
