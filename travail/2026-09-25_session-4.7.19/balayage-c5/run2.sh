#!/bin/bash
# Bilan des curseurs, complément : courbe de chainMm et combinaison des deux desserrages sans faux.
...
for cfg in "chainMm20:--option chainMm=20" "chainMm30:--option chainMm=30" "chain15guard40:--option chainMm=15 --option guardMm=40"; do
  name=${cfg%%:*}; O=${cfg#*:}
  [ -f $S/sweep/$name-done ] && continue
  echo "$(date +%T) $name début" >> $S/sweep/progress2.log
  ...
  touch $S/sweep/$name-done; echo "$(date +%T) $name fin" >> $S/sweep/progress2.log
done
echo FIN >> $S/sweep/progress2.log
